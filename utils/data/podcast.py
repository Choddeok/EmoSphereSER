...
def load_cat_adv_emo_label(label_path, dtype):
    label_df = pd.read_csv(label_path, sep=",")
    cur_df = label_df[label_df["Split_Set"] == SPLIT_MAP[dtype]]
    cur_utts = cur_df["FileName"].to_numpy()
    cur_dim_labs = cur_df[["EmoAct", "EmoDom", "EmoVal"]].to_numpy()
    cur_cat_labs = cur_df[["Angry", "Sad", "Happy", "Surprise", "Fear", "Disgust", "Contempt", "Neutral"]].to_numpy()
    return cur_utts, cur_dim_labs, cur_cat_labs

def load_cat_sev_adv_emo_label(label_path, dtype):
    label_df = pd.read_csv(label_path, sep=",")
    cur_df = label_df[label_df["Split_Set"] == SPLIT_MAP[dtype]]
    cur_utts = cur_df["FileName"].to_numpy()
    cur_dim_labs = cur_df[["EmoAct", "EmoDom", "EmoVal"]].to_numpy()
    cur_cat_labs = cur_df[["Angry", "Sad", "Happy", "Surprise", "Fear", "Disgust", "Contempt", "Neutral"]].to_numpy()
    cur_sev_labs = cur_df[["I", "II", "III", "IV", "V", "VI", "VII", "VIII"]].to_numpy()
    return cur_utts, cur_dim_labs, cur_cat_labs, cur_sev_labs

def load_sev_adv_emo_label(label_path, dtype):
    label_df = pd.read_csv(label_path, sep=",")
    cur_df = label_df[label_df["Split_Set"] == SPLIT_MAP[dtype]]
    cur_utts = cur_df["FileName"].to_numpy()
    cur_dim_labs = cur_df[["EmoAct", "EmoDom", "EmoVal"]].to_numpy()
    cur_sev_labs = cur_df[["I", "II", "III", "IV", "V", "VI", "VII", "VIII"]].to_numpy()
    return cur_utts, cur_dim_labs, cur_sev_labs

def load_45_adv_emo_label(label_path, dtype):
    label_df = pd.read_csv(label_path, sep=",")
    cur_df = label_df[label_df["Split_Set"] == SPLIT_MAP[dtype]]
    cur_utts = cur_df["FileName"].to_numpy()
    cur_dim_labs = cur_df[["EmoAct", "EmoDom", "EmoVal"]].to_numpy()
    cur_sev_labs = cur_df[[f"R{i}" for i in range(0, 33)]].to_numpy()
    return cur_utts, cur_dim_labs, cur_sev_labs

def load_30_adv_emo_label(label_path, dtype):
    label_df = pd.read_csv(label_path, sep=",")
    cur_df = label_df[label_df["Split_Set"] == SPLIT_MAP[dtype]]
    cur_utts = cur_df["FileName"].to_numpy()
    cur_dim_labs = cur_df[["EmoAct", "EmoDom", "EmoVal"]].to_numpy()
    cur_sev_labs = cur_df[[f"R{i}" for i in range(0, 73)]].to_numpy()
    return cur_utts, cur_dim_labs, cur_sev_labs

def load_60_adv_emo_label(label_path, dtype):
    label_df = pd.read_csv(label_path, sep=",")
    cur_df = label_df[label_df["Split_Set"] == SPLIT_MAP[dtype]]
    cur_utts = cur_df["FileName"].to_numpy()
    cur_dim_labs = cur_df[["EmoAct", "EmoDom", "EmoVal"]].to_numpy()
    cur_sev_labs = cur_df[[f"R{i}" for i in range(0, 19)]].to_numpy()
    return cur_utts, cur_dim_labs, cur_sev_labs

def load_180_adv_emo_label(label_path, dtype):
    label_df = pd.read_csv(label_path, sep=",")
    cur_df = label_df[label_df["Split_Set"] == SPLIT_MAP[dtype]]
    cur_utts = cur_df["FileName"].to_numpy()
    cur_dim_labs = cur_df[["EmoAct", "EmoDom", "EmoVal"]].to_numpy()
    cur_sev_labs = cur_df[[f"R{i}" for i in range(0, 4)]].to_numpy()
    return cur_utts, cur_dim_labs, cur_sev_labs
