...
class LogManager:
    ...
    def add_torch_stat2(self, stat_type, stat):
        assert stat_type in self.log_book, "Wrong stat type"
...
def CCC_loss(pred, lab, m_lab=None, v_lab=None, is_numpy=False):
    ...
    corr = torch.sum(d_pred * d_lab, 0) / (
        torch.sqrt(torch.sum(d_pred ** 2, 0) + eps) * torch.sqrt(torch.sum(d_lab ** 2, 0) + eps)
    )
    s_pred = torch.std(pred, 0, unbiased=False)
    s_lab = torch.std(lab, 0, unbiased=False)

    ccc = (2 * corr * s_pred * s_lab) / (v_pred + v_lab + (m_pred[0] - m_lab[0]) ** 2 + eps)   
    return ccc
...
def Focal_loss(inputs, targets, alpha):
