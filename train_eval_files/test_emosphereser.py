...
config_path = "configs/config_balance_dim_cat_sev.json"
with open(config_path, "r") as f:
    config = json.load(f)
audio_path = config["wav_dir"]
label_path = config["label_path"]

# files_test3 = [filename for filename in os.listdir(audio_path) if 'dev' in filename]
...
ser_model = net.EmotionRegression(feat_dim, args.head_dim, 1, 3, dropout=0.5)
##############################################
ser_model.load_state_dict(torch.load(MODEL_PATH+"/final_dim_ser.pt"))
